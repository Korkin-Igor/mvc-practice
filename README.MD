...
`git clone https://github.com/korkin-igor/mvc-practice`
...
2. USER - логин пользователя под которым будет запущен веб-сервер в контейнере с php. Если пустой, то логин текущий логин пользователя или `user` (зависит от операционной системы). Также такой пользователь будет создан для доступа к базе данных в контейнере с mariadb;
